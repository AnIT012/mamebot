...
家族4人で継続運用中。実際に「使われ続ける」中で得た学び：

- **「使われる」は機能数で決まらない**：高機能なWebアプリ（Irori）より、LINEに溶け込んだ通知のほうが定着した。生活動線に入ることが最重要だと実感した。
- **通知は「少なく・まとめて」が正義**：個別チャットで入力 → グループに集約する設計で、家族LINEを荒らさずに情報共有できた。「今日ごはんいる？」「お風呂洗った？」といった確認のやり取りが目に見えて減った。
- **運用して初めて出るバグ**：リマインダーの重複送信など、実運用でしか見えない問題に対処し、冪等性（同じ通知を二度送らない）の重要性を学んだ。
